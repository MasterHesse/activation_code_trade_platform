...
public class MockPaymentProcessor implements PaymentProcessor {
    ...
    public PaymentCloseResult close(Order order) {
        if (order.getPaymentStatus() == PaymentStatus.PAID) {
            return PaymentCloseResult.paid("模拟支付订单已支付，无需关单");
        }

        return PaymentCloseResult.closed("模拟支付订单已关闭");
    }

    private void validateOrder(Order order) {
        if (!StringUtils.hasText(order.getPaymentRequestNo())) {
            throw new BusinessException(HttpStatus.BAD_REQUEST, "模拟支付失败：paymentRequestNo 为空");
        }

        BigDecimal payAmount = order.getPayAmount();
        if (payAmount == null || payAmount.compareTo(BigDecimal.ZERO) <= 0) {
            throw new BusinessException(HttpStatus.BAD_REQUEST, "模拟支付失败：payAmount 必须大于 0");
        }
    }
    ...
    private String resolveChannelTradeNo(Order order) {
        ...
        if (StringUtils.hasText(order.getPaymentRequestNo())) {
            return "MOCK" + order.getPaymentRequestNo();
        }
        return null;
    }
}